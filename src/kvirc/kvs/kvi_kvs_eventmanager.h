...
protected:
	...
	QList<KviKvsEventHandler*> *     m_rawEventTable[KVI_KVS_NUM_RAW_EVENTS];
public:
	...
	QList<KviKvsEventHandler*> * appHandlers(unsigned int uEvIdx){ return m_appEventTable[uEvIdx].handlers(); };
	
	bool hasRawHandlers(unsigned int uEvIdx){ return m_rawEventTable[uEvIdx]; };
	QList<KviKvsEventHandler*> * rawHandlers(unsigned int uEvIdx){ return m_rawEventTable[uEvIdx]; };
	...
	bool triggerHandlers(QList<KviKvsEventHandler*> * pHandlers,KviWindow *pWnd,KviKvsVariantList *pParams);
