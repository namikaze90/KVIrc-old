...
: QObject(0)
{
	m_pInstanceTree = new QList<KviOptionsWidgetInstanceEntry *>;
	...
	if [ -f "$CLASSDIR/$2" ]; then
		...
		for achild in $LEVEL; do
			...
			if [ -f "$CLASSDIR/$achild" ]; then
				echo "" >> $TARGET
				echo "$3	e$1->pChildList = new QList<KviOptionsWidgetInstanceEntry *>;" >> $TARGET
				NEXTLEVEL=`expr $1 + 1`
				addchildren $NEXTLEVEL $achild "$3	"
			else
				echo "$3	e$1->pChildList = NULL;" >> $TARGET
			fi
		done
	fi
}
...
void KviOptionsInstanceManager::deleteInstanceTree(QList<KviOptionsWidgetInstanceEntry *> * l)
{
	if(!l)return;
	foreach(KviOptionsWidgetInstanceEntry * e,(*l))
	{
		if(e->pWidget)
		{
...
KviOptionsWidget * KviOptionsInstanceManager::getInstance(KviOptionsWidgetInstanceEntry * e,QWidget * par)
{
	...
	if(e->bIsContainer)
	{
		...
		if(e->pChildList)
		{
			...
			foreach(KviOptionsWidgetInstanceEntry * e2,(*(e->pChildList)))
			{
				// add only non containers and widgets not explicitly marked as noncontained
				if((!e2->bIsContainer) && (!e2->bIsNotContained))
				{
					QList<KviOptionsWidgetInstanceEntry *>::Iterator it = tmp.begin();
					int idx = 0;
					while(it != tmp.end())
					{
						KviOptionsWidgetInstanceEntry * ee = *it;
			...
			while(ii >= 0)
			{
				KviOptionsWidgetInstanceEntry * e3 = tmp.at(ii);
...
KviOptionsWidgetInstanceEntry * KviOptionsInstanceManager::findInstanceEntry(const QObject * ptr,QList<KviOptionsWidgetInstanceEntry *> * l)
{
	if(l)
	{
		foreach(KviOptionsWidgetInstanceEntry * e,(*l))
...
KviOptionsWidgetInstanceEntry * KviOptionsInstanceManager::findInstanceEntry(const char * clName,QList<KviOptionsWidgetInstanceEntry *> * l)
{
	if(l)
	{
		foreach(KviOptionsWidgetInstanceEntry * e,(*l))
